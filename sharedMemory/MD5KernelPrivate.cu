...
__global__ void
calculateHashSum(unsigned char *digest, char *word, int workingBufferLength, int lenght, volatile bool *kernel_end) {

    bool done;
    uint32_t workingBuffer[MAX_WORKING_BUFFER_SIZE / 4];
    //init working buffer
...
#pragma unroll
        ...
        if (mdBuffer.a == reinterpret_cast<uint32_t *>(digest)[0] &&
            ...
            memcpy(word, workingBuffer, lenght * sizeof(char));
            reinterpret_cast<uint32_t *>(word)[0] += (blockIdx.x * 256) | threadIdx.x;
            *kernel_end = true;
        }
